...
#+begin_src json :results file silent :file experiments/time-conf.json
{
    ...
    "parameters": [	
	{
		...
		"time-ablation/rules-t-100000.json",
		"time-ablation/rules-t-200000.json"
	    ]
	},
	{
	    "name": "cost-filter",
...
ssh ubuntu@3.15.0.175 \
    "cd diospyros && \
     bash -cl 'experiments/testbed.py \
     	   experiments/time-conf.json experiments/ruler-time-ablation2 \
     1>experiments/ruler-time-ablation2.out 2>&1 &' &"
#+end_src
...
#+begin_src shell :dir (magit-toplevel) :results output
cd experiments
./extract.py time-conf.json ruler-time-ablation4 ruler-time-ablation4.csv
#+end_src
...
data <- read.csv("ruler-time-ablation3.csv")
#+end_src
...
data %>%
  ...
  ## filter(no.dup.vars == "True") %>%
  filter(bench == "q-prod") %>%
  filter(variable == "cost" | variable == "xtime" | variable == "xphase_2_n") %>%
  select(id, rules, cost.filter, sub.prog, variable, value)
#+end_src

#+RESULTS:
|  id | rules                             | cost.filter | sub.prog | variable  | value |
|-----+-----------------------------------+-------------+----------+-----------+-------|
|  33 | time-ablation/rules-t-60.json     |           5 | False    | phase_2_n |    17 |
|  87 | time-ablation/rules-t-600.json    |           5 | False    | phase_2_n |     5 |
| 141 | time-ablation/rules-t-6000.json   |           5 | False    | phase_2_n |     5 |
| 195 | time-ablation/rules-t-60000.json  |           5 | False    | phase_2_n |   255 |
| 249 | time-ablation/rules-t-100000.json |           5 | False    | phase_2_n |     5 |
| 303 | time-ablation/rules-t-200000.json |           5 | False    | phase_2_n |       |
| 357 |                                   |           5 | False    | phase_2_n |       |

...
#+RESULTS:
: RUST_LOG=info cargo run --manifest-path ruler/Cargo.toml --release --bin dios -- synth --dios-config configs/vec4_baseline.json --variables 6 --iters 2 --vector-size 3 --eqsat-iter-limit 3 --eqsat-time-limit 60 --num-fuzz 6 --abs-timeout 60 --outfile vec/test.json

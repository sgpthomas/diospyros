...
#+RESULTS:
: RUST_LOG=info cargo run --manifest-path ruler/Cargo.toml --release --bin dios -- synth --dios-config configs/vec4_baseline.json --variables 8 --iters 2 --vector-size 4 --eqsat-iter-limit 2 --eqsat-time-limit 30 --num-fuzz 8 --abs-timeout 240 --outfile vec/test.json

#+begin_src emacs-lisp :dir (magit-toplevel) :results none

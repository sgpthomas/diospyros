...
RUST_BACKTRACE=1 cargo run --release --manifest-path src/dios-egraphs/Cargo.toml \
      test-extr/spec.rkt \
      --phase pre-compile --phase compile \
      --new-egraph \
      --vector-width 2 \
      --split-phase syntax \
      --rules vec/test4.json \
      ...
      --node-limit 50000000 --iter-limit 1
#+end_src

#+begin_src shell :var dir=(ec2/get-ip "exp1") :results silent
rsync ubuntu@"$dir":~/diospyros/test.pdf ~/Downloads
open ~/Downloads/test.pdf
#+end_src

** Sketch of the "top-down" cost function algorithm

For each eclass, for each node in that eclass. check if children match the children of rules. score of child is number of rules that match it.

for each eclass, extract the node that has the highest score.

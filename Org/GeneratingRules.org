...
The idea is that there are two types of rules, rules that improve the cost, and rules that increase the graph size and allow cost improving rules to apply. The first type of rules are easy to discover, you can use this idea of the cost differential (cost of lhs minus cost of rhs).

The second type of rules are much harder to get a grasp on. My first idea which I want to try is take the existing set of rules that I know are good, add a single rule from some candidate set, and track what it does to the size vs. cost of the graph.

** Most complicated version to simplest version

The most advanced version of this algorithm assumes 0 rules to start off with and has to discover all the good rules by itself. This seems hard when we have a hundred thousand candidate rules. This would be a very cool problem to solve though.

The simplest version of the algorithm is determining if the addition of a single rule is worthwhile. Let's start with this version. Here's the game plan. Let's start with just the rules that convert a vec of ops into a single VecOp and then see if we can come up with an algo that determines if a single additional rule is worthwhile to add or not.

** Something

The vague idea is that maybe we don't need to reify the ruler egraph full of rules until later in the saturation process. We can use particular programs to extract rules that will be good for it?

Things we definitely can do:
- fill an egraph with rules
- find left hand sides with low cost right hand sides (these can represent combinations of rules)

Is this actually better than saturating the graph with the rules directly? Probably because with 130,000 rules even a single iteration is prohibitively expensive and saturation would be infeasible.
...
** Diospyros might be a substantively different domain than the ones measured in the ruler evaluation

The reason I think this is that Dios is really translating from one domain to a different one. Namely it is translating from a language of scalar operations to a language of vector operations. This makes Dios a more compilery task than something like Herbie or bitvectors.

I think that implications of this is that their approach is a little too general. It justifies this idea of trying to bring a little more knowledge of the domains into the rule searching process.

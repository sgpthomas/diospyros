...
pub fn vec_with_op(vec_width: usize, op: &String, pre: &String) -> String {
    let joined = ids_with_prefix(pre, vec_width).join(" ");
    format!("({} {})", op, joined)
}

pub fn vec_with_var(vec_width: usize, pre: &String) -> String {
    vec_with_op(vec_width, &"Vec".to_string(), pre)
}
...
pub fn vec_fold_op(
    ...
    let mut ops: Vec<String> = Vec::with_capacity(vec_width);
...
pub fn vec_map_op(vec_width: usize, op: &String, pre: &String) -> String {
    let mut ops: Vec<String> = Vec::with_capacity(vec_width);
    for i in 0..vec_width {
